...
class HiveWarehouseBuilderTest extends SessionTestBase {
    ...
    void testAllConfConfig() {
        session.conf().set(com.hortonworks.spark.sql.hive.llap.common.HWConf.USER.qualifiedKey, TEST_USER);
        session.conf().set(com.hortonworks.spark.sql.hive.llap.common.HWConf.PASSWORD.qualifiedKey, TEST_PASSWORD);
        session.conf().set(com.hortonworks.spark.sql.hive.llap.common.HWConf.DBCP2_CONF.qualifiedKey, TEST_DBCP2_CONF);
        session.conf().set(com.hortonworks.spark.sql.hive.llap.common.HWConf.MAX_EXEC_RESULTS.qualifiedKey, TEST_EXEC_RESULTS_MAX);
        session.conf().set(com.hortonworks.spark.sql.hive.llap.common.HWConf.DEFAULT_DB.qualifiedKey, TEST_DEFAULT_DB);
        HiveWarehouseSessionState sessionState =
                HiveWarehouseBuilder
                        .session(session)
                        .sessionStateForTest();
